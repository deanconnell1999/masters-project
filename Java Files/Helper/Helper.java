...
public class Helper {
	...
	public static void printResults(
		...
		if (outputInCsv) {
			...
			titles.append("experimentName" + delimeter);
			titles.append("loadType" + delimeter);
			titles.append("vmAllocationPolicy"+ delimeter);
			titles.append("vmSelectionPolicy" + delimeter);
			titles.append("parameter" + delimeter);
			// titles.append(parseExperimentName(experimentName));
			titles.append("numberOfHosts" + delimeter);
			titles.append("numberOfVms" + delimeter);
			titles.append("totalSimulationTime" + delimeter);
			titles.append("energy" + delimeter);
			titles.append("numberOfMigrations" + delimeter);
			titles.append("sla" + delimeter);
			titles.append("slaTimePerActiveHost" + delimeter);
			titles.append("slaDegradationDueToMigration" + delimeter);
			titles.append("slaOverall" + delimeter);
			titles.append("slaAverage" + delimeter);
			...
			titles.append("numberOfHostShutdowns" + delimeter);
			titles.append("meanTimeBeforeHostShutdown" + delimeter);
			titles.append("stDevTimeBeforeHostShutdown" + delimeter);
			titles.append("meanTimeBeforeVmMigration" + delimeter);
			titles.append("stDevTimeBeforeVmMigration" + delimeter);
			
			
			data.append(experimentName + delimeter);
			data.append(parseExperimentName(experimentName));
			// data.append(parameter + delimeter);
			...
			if (datacenter.getVmAllocationPolicy() instanceof PowerVmAllocationPolicyMigrationAbstract) {
				...
				titles.append("executionTimeVmSelectionMean" + delimeter);
				titles.append("executionTimeVmSelectionStDev" + delimeter);
				titles.append("executionTimeHostSelectionMean" + delimeter);
				titles.append("executionTimeHostSelectionStDev" + delimeter);
				titles.append("executionTimeVmReallocationMean" + delimeter);
				titles.append("executionTimeVmReallocationStDev" + delimeter);
				titles.append("executionTimeTotalMean" + delimeter);
				titles.append("executionTimeTotalStDev" + delimeter);
			...
			titles.append("\n");
			data.append("\n"); 
			String[] noParameterExp = Arrays.copyOfRange(experimentName.split("_"),0,3);
			String fileName = String.join("_", noParameterExp);
			System.out.println(fileName);
        	File file = new File(outputFolder + "/stats/" + fileName + "_stats.csv");
        	if (file.exists()) {
	            try {
	...
	/**
	 * Gets the running sla total.
	 *
	 * @param vms the migratable vms
	 * @return the accumulated sla metric
	 *
	 * @author Dean Connell
	 */

	public static List<Double> getSla(List<PowerVm> vms) {
		...
		for (Vm vm : vms) {
			...
			double slaDistance = (vmTotalRequested - vmTotalAllocated) / vmTotalRequested; 
			metrics.add(slaDistance);
		}
		return metrics;
	}
